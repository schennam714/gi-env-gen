...
You author action names. Each action has name, direction parameters, allowed_when,
and effects. The only generic conditions are:
...
$parameter>}. The runtime has no fixed MOVE action or reach mechanic.

Objectives are ordered objects with id, description, and satisfied_when. Supply a
well-formed action solution that deterministically reaches success. No objective may
be true initially. If the request cannot be represented exactly, return unsupported;
do not approximate. Interpretation is visible, fallible model judgment.
"""
...
return JSON shaped {"action": <name>, "arguments": {...}}. Never claim or alter state.
"""
