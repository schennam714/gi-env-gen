...
def test_reviewer_cli_separates_program_proof_and_independent_acting_evidence(
    ...
    builder_response = reach_build_response()

    class BuilderProviderFake:
        def generate_build(self, request: Any) -> dict[str, Any]:
            return deepcopy(builder_response)

    class ActorProviderFake:
        def __init__(self) -> None:
            self.actions = deepcopy(builder_response["solution"])
            self.observations: list[dict[str, Any]] = []
    ...
    actor = ActorProviderFake()
    requested_models: list[str] = []

    def provider_factory(*, model: str) -> object:
        requested_models.append(model)
        return BuilderProviderFake() if model == "gpt-5.6" else actor
    ...
    assert exit_code == 0
    assert requested_models == ["gpt-5.6", "reviewer-actor-fake"]
    output = capsys.readouterr().out
    assert "Builder: gpt-5.6 · Actor: reviewer-actor-fake" in output
    ...
    assert generation["builder_model"] == "gpt-5.6"
    assert acting["actor_model"] == "reviewer-actor-fake"
    assert summary["builder_model"] == "gpt-5.6"
    assert summary["actor_model"] == "reviewer-actor-fake"
    ...
    assert acting["steps"][0]["observation"] == json.loads(
        json.dumps(actor.observations[0])
    )
...
def test_generation_stopped_terminal_retains_builder_and_actor_models(
    tmp_path: Path,
    monkeypatch: Any,
) -> None:
    rejected = reach_build_response()
    ...
    assert exit_code == 3
    final_frame = terminal_output.getvalue()[
        terminal_output.getvalue().rfind("Generation stopped") :
    ]
    assert "Builder: gpt-5.6 · Actor: economical-actor" in final_frame
...
def test_terminal_dashboard_uses_normal_buffer_and_leaves_final_frame(
    ...
    assert "Builder: gpt-5.6" in final_frame
    assert "Actor: reviewer-fake" in final_frame
