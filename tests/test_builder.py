...
from gi_env_gen.builder import (
    AcceptedBuild,
    BuildRequest,
    GenerationFailed,
    ProviderFailed,
...
def test_builder_stops_after_three_rejections_with_attempt_attribution() -> None:
    provider = FakeProvider(invalid_reference_response(), invalid_reference_response(), invalid_reference_response())
...
def test_builder_never_repairs_geometry_or_rules_itself() -> None:
    ...
    provider = FakeProvider(malformed, malformed, malformed)
